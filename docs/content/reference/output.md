...
describes what users get. Pick a format with `-o`, or let fptshop choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
fptshop <command> -o table   # aligned columns for reading
fptshop <command> -o jsonl   # one JSON object per line, for piping
fptshop <command> -o json    # a single JSON array
fptshop <command> -o csv     # spreadsheet friendly
fptshop <command> -o tsv     # tab-separated
fptshop <command> -o url     # just the URL column
fptshop <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
fptshop <command> --fields id,title,url
```
...
```bash
fptshop <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
fptshop <command>            # a table, because this is a terminal
fptshop <command> | wc -l    # JSONL, because this is a pipe
```
